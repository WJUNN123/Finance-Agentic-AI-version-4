...
def render_summary_dashboard(result: Dict, horizon_days: int):
    """
    Full-width layout using Streamlit native components (no complex HTML)
    """
    ...
    # ========================================================================
    # ROW 1: HEADER - USING STREAMLIT COLUMNS
    # ========================================================================
    
    # Header background
    st.markdown(f"""
    <div style="background: linear-gradient(135deg, {rec_color}18 0%, transparent 100%); 
                 border-left: 6px solid {rec_color}; border-radius: 12px; 
                 padding: 20px 24px; margin-bottom: 20px;"></div>
    """, unsafe_allow_html=True)
    
    # Create 5 columns for header
    h1, h2, h3, h4, h5 = st.columns([1.2, 0.8, 1, 0.7, 0.7])
    
    with h1:
        st.markdown(f"### {coin_symbol}")
        st.markdown(f"<h1 style='font-size:2.2rem;margin:0;'>${current_price:,.2f}</h1>", unsafe_allow_html=True)
        change_color = "#ef4444" if price_change_24h < 0 else "#10b981"
        st.markdown(f"<span style='color:{change_color};font-weight:600;'>{price_change_24h:+.2f}% • 24h</span>", 
                   unsafe_allow_html=True)
    
    with h2:
        st.markdown(f"<div style='text-align:center;'><span style='font-size:3rem;'>{rec_emoji}</span></div>", 
                   unsafe_allow_html=True)
    
    with h3:
        st.markdown(f"<h2 style='font-size:2rem;color:{rec_color};margin:0;font-weight:700;'>{rec_text}</h2>", 
                   unsafe_allow_html=True)
        st.caption(f"{confidence:.0%} Confidence • {horizon_days}-Day")
    
    with h4:
        st.metric(f"{horizon_days}d Target", f"${ensemble_pred:,.0f}", f"{roi_pct:+.1f}%")
    
    with h5:
        agreement_label = "High" if model_agreement > 0.7 else "Mod" if model_agreement > 0.6 else "Low"
        st.metric("Agreement", f"{model_agreement:.0%}", agreement_label)
    
    st.markdown("<br>", unsafe_allow_html=True)
    
    # ========================================================================
    # ROW 2: CHART (LEFT) | ANALYSIS (RIGHT)
    # ========================================================================
    
    chart_col, analysis_col = st.columns([1.4, 1], gap="large")
    ...
    with chart_col:
        ...
        if forecast_table:
            ...
            if ensemble_preds:
                ...
                fig.add_hline(y=min_forecast*0.98, line_dash="dot", line_color="#10b981", 
                             line_width=2, annotation_text="Support", annotation_position="left")
                fig.add_hline(y=max_forecast*1.02, line_dash="dot", line_color="#f59e0b",
                             line_width=2, annotation_text="Resistance", annotation_position="left")
        ...
        fig.update_layout(
            ...
            yaxis=dict(showgrid=True, gridcolor='#334155', color='#94a3b8', tickformat='$,.0f'),
        ...
        with m3:
            vol_label = "High" if volatility > 0.08 else "Med" if volatility > 0.05 else "Low"
            st.metric("Volatility", f"{volatility*100:.1f}%", vol_label)
    ...
    with analysis_col:
        ...
        if rec_text == "BUY":
            st.success("📈 **Entry Strategy**")
            st.write(f"🎯 **Entry:** ${technical.get('support', current_price*0.98):,.0f}")
            st.write(f"📊 **Current:** ${current_price:,.0f}")
            st.write(f"🛡️ **Stop:** ${technical.get('support', current_price*0.98)*0.95:,.0f}")
        elif rec_text == "SELL":
            st.error("📉 **Exit Strategy**")
            st.write(f"🎯 **Exit:** ${technical.get('resistance', current_price*1.02):,.0f}")
            st.write(f"📊 **Current:** ${current_price:,.0f}")
            st.write(f"🛡️ **Stop:** ${current_price*1.05:,.0f}")
        else:
            st.warning("⏸️ **Wait Strategy**")
            st.write(f"⏸️ **Action:** Hold Position")
            st.write(f"👀 **Watch:** ${technical.get('support', current_price*0.98):,.0f} - ${technical.get('resistance', current_price*1.02):,.0f}")
            st.write(f"✅ **Review:** Daily")
        ...
        **💰 Position Info**
        ...
        {sentiment_emoji} Sentiment: {sentiment_confidence:.0%}  
